...
func main() {
	...
	mux.Handle("GET /templates/{$}", requireAuth(handlers.HandleTemplatesPage(nc, handlerLogger)))
	mux.Handle("POST /templates", requireAuth(handlers.HandleCreateTemplate(nc, handlerLogger)))
	mux.Handle("GET /templates/{id}", requireAuth(handlers.HandleTemplateDetail(nc, handlerLogger)))
	mux.Handle("DELETE /templates/{id}", requireAuth(handlers.HandleDeleteTemplate(nc, handlerLogger)))
	mux.Handle("POST /templates/{id}/title", requireAuth(handlers.HandleUpdateTemplateTitle(nc, handlerLogger)))
	mux.Handle("POST /templates/{id}/items", requireAuth(handlers.HandleCreateTemplateItem(nc, handlerLogger)))
	mux.Handle("DELETE /templates/{id}/items/{itemId}", requireAuth(handlers.HandleDeleteTemplateItem(nc, handlerLogger)))
