...
func (s *Service) CreateTemplate(
	...
	s.logger.Info("template created", "template_id", template.ID, "user_id", userID)
...
func (s *Service) GetTemplatesByUser(ctx context.Context, userID uuid.UUID) ([]db.GetTemplatesByUserRow, error) {
	...
	s.logger.Info("templates retrieved", "user_id", userID, "count", len(templates))
...
func (s *Service) UpdateTemplateTitle(
	...
	if err != nil {
		s.logger.Error("database error", "error", err, "template_id", templateID, "user_id", userID)
		return nil, translateDBError(err)
	}

	s.logger.Info("template title updated", "template_id", templateID, "user_id", userID)
...
func (s *Service) DeleteTemplate(ctx context.Context, templateID, userID uuid.UUID) error {
	...
	s.logger.Info("template deleted", "template_id", templateID, "user_id", userID)
...
func (s *Service) CreateTemplateItem(
	...
	s.logger.Info("template item created", "item_id", item.ID, "template_id", templateID, "user_id", userID)
...
func (s *Service) GetTemplateItems(
	...
	s.logger.Info("template items retrieved", "template_id", templateID, "user_id", userID, "count", len(items))
...
func (s *Service) DeleteTemplateItem(ctx context.Context, itemID, userID uuid.UUID) error {
	...
	s.logger.Info("template item deleted", "item_id", itemID, "user_id", userID)
...
func (s *Service) UseTemplate(
	...
	for _, ti := range templateItems {
		...
		if err != nil {
			s.logger.Error("database error", "error", err, "template_id", templateID, "list_id", list.ID, "user_id", userID)
			return nil, translateDBError(err)
		}
	}

	s.logger.Info("template used", "template_id", templateID, "list_id", list.ID, "user_id", userID, "items_count", len(templateItems))
